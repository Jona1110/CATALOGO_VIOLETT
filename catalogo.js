...
// 2. Interceptar el evento de instalación (Aparece si el dispositivo cumple los requisitos)
window.addEventListener('beforeinstallprompt', (e) => {
    // Prevenir que el navegador muestre su propio prompt predeterminado
    e.preventDefault();
...
// 3. Manejar el clic en el botón de instalación
if (btnInstalar) {
    btnInstalar.addEventListener('click', async () => {
        ...
        // Si el usuario acepta, ocultar el botón
        if (outcome === 'accepted') {
...
// 4. Escuchar cuando la app ya se instaló exitosamente
window.addEventListener('appinstalled', () => {
    console.log('¡Catálogo Violett instalado con éxito!');
    if (btnInstalar) {
        btnInstalar.classList.add('hidden');
    }
});